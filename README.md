...
- **Advanced SSRF Protection**: Outgoing webhooks are validated by resolving DNS and checking if the resulting IP is in a private or reserved range (loopback, private subnets, link-local, etc.).
- **Per-Key Rate Limiting**: API calls are rate-limited per API Key (default 300 req/min) to ensure fair usage and prevent abuse.
- **HMAC Signing**: All outgoing webhooks are signed with HMAC-SHA256.
...
│   │   └── apiKey.js
│   ├── queues/
│   │   ├── BaseConsumer.js     # Abstract RabbitMQ worker logic
...
│   │   └── apiKeys.js
│   ├── services/
│   │   ├── notificationService.js # Unified business logic
│   │   ├── emailService.js     # AWS SES client
│   │   └── webhookService.js   # SSRF-safe (DNS-validated) dispatcher
│   ├── utils/
