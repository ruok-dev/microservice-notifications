...
/**
 * Rate limiter for API keys — per API Key ID if present, otherwise IP.
 */
const apiKeyRateLimiter = rateLimit({
  windowMs: 1 * 60 * 1000, // 1 minute
  max: 300, // 300 requests per minute per key
  standardHeaders: true,
  legacyHeaders: false,
  keyGenerator: (req) => (req.auth?.subject ? `key:${req.auth.subject}` : req.ip),
  handler: (_req, _res, next) => next(new RateLimitError()),
});
